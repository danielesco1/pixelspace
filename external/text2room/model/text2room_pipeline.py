...
from text2room.model.utils.utils import (
...
from text2room.model.mesh_fusion.util import (
    get_pinhole_intrinsics_from_fov,
    torch_to_trimesh,
    get_camera
)
from text2room.model.mesh_fusion.render import (
...
from text2room.model.iron_depth.predict_depth import load_iron_depth_model, predict_iron_depth
from text2room.model.depth_alignment import depth_alignment
from text2room.model.trajectories import trajectory_util, pose_noise_util
from text2room.model.trajectories.convert_to_nerf_convention import convert_pose_to_nerf_convention, convert_pose_from_nerf_convention
...
class Text2RoomPipeline(torch.nn.Module):
    ...
    def setup_start_image(self, first_image_pil, offset):
        ...
        ##print(f"start_image: {self.current_image.shape}")
    ...
    def predict_depth(self):
        ...
        predicted_depth, _ = predict_iron_depth(
            image=self.current_image_pil.resize((512,512)),
            K=self.K,
        ...
        #print(f"depth {predicted_depth.shape}")
    ...
    def add_vertices_and_faces(self, predicted_depth):
        if self.inpaint_mask.sum() == 0:
            ...
            return
        #print(f"current image: {self.current_image.shape}")
        #print(f"depth: {predicted_depth.shape}")
    ...
    def get_next_pose_in_trajectory(self, i=0):
        #print(f"pose i: {i} {self.trajectory_dict} {self.trajectory_fn}")
    ...
    def refine_image(self,init_image):
        #print("Refining image ....")
        prompt = self.args.prompt
        # pass prompt and image to pipeline
        image = self.refiner_pipe(prompt, image=init_image, strength=0.3).images[0]
    ...
    def inpaint(self, rendered_image_pil, inpaint_mask_pil):
        ...
        inpainted_image_pil = self.inpaint_pipe(
                 prompt=self.args.prompt, 
                 negative_prompt=self.args.negative_prompt,
                 image=rendered_image_pil.resize((1024,1024)), 
                 mask_image=inpaint_mask_pil.resize((1024,1024)), 
                 num_inference_steps=self.args.num_inference_steps, 
                 guidance_scale=self.args.guidance_scale,
                 #strength= 0.8,
                 num_images_per_prompt=1,
        ...
        inpainted_image_pil =  inpainted_image_pil.resize((512,512))
        #print(f"inpainted pil: {inpainted_image_pil.size}")
    ...
    def add_next_image(self, pos, offset, save_files=True, file_suffix=""):
        ...
        #print(f"pred_depth : {predicted_depth.shape} rendered {rendered_depth_pil.size}")
    ...
    def project_and_inpaint(self, pos=0, offset=0, save_files=True, file_suffix="", inpainted_image_pil=None):
        ...
        if "adaptive" in self.trajectory_dict:
            #print(f"adaptive: {self.trajectory_dict}")
        ...
        #print(f"inpainted_img: {inpainted_image_pil.size}")
        inpainted_image_pil = inpainted_image_pil.resize((512,512))
        
        #can add module to img2img for cleaning up the inpainted image
        
        #inpainted_image_pil,comparison_image = self.refine_image(inpainted_image_pil)
        
        combination_img = make_image_grid([inpainted_image_pil,inpaint_mask_pil,self.depth_pil], rows=1, cols=3)
        ...
        if save_files:
            ...
            save_image(combination_img, f"rgb{file_suffix}", offset + pos, self.args.combined_image_path)
    ...
    def forward(self, pos=0, offset=0, save_files=True):
        ...
        camera = get_camera(self.world_to_cam, self.args.fov)
        self.seen_cameras.append(camera)
